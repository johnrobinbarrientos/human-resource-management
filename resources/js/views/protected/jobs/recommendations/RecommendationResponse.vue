                 ...
                 <div class="card">
                    <div class="card-body">
                        ...
                        <div v-if="status == 'accepted'">
                            ...
                            <h4 class="text-center text-success">Job Accepted</h4>
                            <p class="text-center">You have accepted the <strong>{{ recommendation.job.title }}</strong>, we will keep in touch with you within 24 hours, Thank you!</p>
                            <br/>
                        </div>
                        <div v-if="status == 'declined'">
                            ...
                            <p class="text-center">You have declined the <strong>{{ recommendation.job.title }}</strong>, Thank you for your response!</p>
                            <br/>
                        </div>
...
<script>
export default {
    name: 'job-recommendation-response',
    props: ['properties'],
    data: function () {
        return {
            ready: true,
    ...
    methods: {
        accept: function () {
            ...
            scope.POST('jobs/posts/'+ job_post_id +'/recommendations/accept/' + token).then(res => {
                if (res.success) {
        ...
        decline: function () {
            ...
            scope.POST('jobs/posts/'+ job_post_id +'/recommendations/decline/' + token).then(res => {
                if (res.success) {
                    scope.recommendation = res.data
