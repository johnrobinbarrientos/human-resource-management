<template>
    <div v-if="ready">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0 font-size-18">Applicants</h4>
        ...
        <div class="card">
         <div class="card-body">
            <div class="row align-items-right">
                <div class="col-md-6">
                    <h4>Candidates</h4>
                    <p>Recommend this job to the list of candidates below</p>
                </div>
                <div class="col-md-6 text-right">
                    <button class="btn btn-primary" @click="sendRecommendToSelectedWorkers()">Send Recommendations</button>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table mb-0 table table-striped table-bordered">
                    ...
                    <tbody>
                    <tr v-for="(match, index) in post.matches" :key="index">
                        <td>
                            <div style="padding-left:30px; padding-bottom:20px;">
                        ...
                        <th>
                            <span v-if="!match.user.first_name || !match.user.first_name">DELETED</span>
                            <span v-else>{{ match.user.first_name }} {{ match.user.last_name }}</span>
                        </th>
                        <th>
                            <template v-if="match.job_recommendation">
                                ...
                                <span v-else-if="match.job_recommendation.status == 'accepted_by_candidate'" class="badge badge badge-soft-success">Applied</span>
                        ...
                        <th>
                            <button v-if="!match.job_recommendation"  @click="recommendJob(match)" style="border-radius:3px; padding:2px 10px; background:#1cb841; color:#fff; font-size:10px; border:none;">
                                RECOMMEND 
                            </button>
...
export default {
    ...
    computed: {
        jobRecommendations() {
            var scope = this
            return scope.post.matches
            return scope.post.matches.filter((item) => {
                // filter: to not include worker who is already hired.
                ...
                return (item.user.company_id == null && 
                    (!item.job_recommendation ||
                        (
                            ...
                            item.job_recommendation.status != 'removed' &&
                            item.job_recommendation.status != 'rehiring'
                        )
                    )
                );
        ...
        workerRecommendations() {
            ...
            return scope.post.matches.filter((item) => {
                ...
                return (item.job_recommendation &&
                    (
                        item.job_recommendation.status == 'accepted_by_candidate' ||
                        item.job_recommendation.status == 'accepted_by_employer' ||
                        item.job_recommendation.status == 'removed' ||
    ...
    methods: {
        ...
        selectAllMatches: function($event) {
            ...
            if($event.target.checked) {
                for (let i = 0; i < scope.post.matches.length; i++) {
                    var current = scope.post.matches[i]
        ...
        recommendJob: function (match) {
            ...
            window.swal.fire({
                ...
                if (result.value) {
                    ...
                    scope.POST('jobs/posts/' + job_post_id + '/recommendations',{ recommendations: recommendations }).then(res => {
                        if (res.success) {
                            window.swal.fire({
                                icon: "success",
                                text: res.message
        ...
        sendRecommendToSelectedWorkers: function(){
            var scope = this
