                       ...
                       <div class="card">
                            <div class="card-body">
                                ...
                                <div v-if="status == 'error'">
                                    ...
                                    <p class="text-center">An error occur while verifying your account, this could be due to invalid token or your account has already been verified.</p>
                                    <br/>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    methods: {
        authenticate: function (token) {
            ...
            scope.axios
            .get(window.API_URL + '/auth/verification/' + token, {})
            .then(response => {
                var data = response.data
                if (data.success) {
                    scope.status = 'verified'
                    setTimeout(function(){
                        scope.$store.dispatch('authenticate',{ token: data.token, redirect: '/dashboard?alert=complete-profile' })
                    },1500);
